...
def T_(x):#字变换组合 32输入32输出
    ...
    a3 = (x >> 0) & 0xff
    # s盒变换
    B = (subbytes(a0)<<24)^(subbytes(a1)<<16)^(subbytes(a2)<<8)^(subbytes(a3)<<0)
    return B^shift(B,2)^shift(B,10)^shift(B,18)^shift(B,24)
...
def T(x):#密钥扩展字变换
    ...
    B = (subbytes(a0) << 24) ^ (subbytes(a1) << 16) ^ (subbytes(a2) << 8) ^ (subbytes(a3) << 0)
    # 循环左移
    return B ^ (shift(B,13)) ^ (shift(B,23))

def KeyExtension(MK):#SM4密钥扩展算法
...
def SM4_enc(x):#SM4加密算法
    X = [0x0 for i in range(36)]
    X[0] = (x >> 96) & 0xffffffff
    X[1] = (x >> 64) & 0xffffffff
    X[2] = (x >> 32) & 0xffffffff
    X[3] = (x >> 0) & 0xffffffff
    for i in range(32):
        X[i+4] = X[i]^T_(X[i+1]^X[i+2]^X[i+3]^K[i+4])#轮函数
